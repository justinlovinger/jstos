...
let
  syncCfgs = map (jstos: jstos.data.sync) (builtins.attrValues config.jstos.users);
  snapshotCfgs = lib.mapAttrs (_: jstos: jstos.data.snapshot) config.jstos.users;
...
lib.mkMerge [
  ...
  (lib.mkIf (builtins.any (cfg: cfg.client.enable || cfg.server.enable) syncCfgs) {
    ...
    services.openssh.settings = lib.mkIf (builtins.any (cfg: cfg.server.enable) syncCfgs) {
      TCPKeepAlive = "no";
